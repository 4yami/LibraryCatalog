...
public class BookDetailsFrame extends JFrame {
    ...
    private String validateBookDetails() {
        ...
        } else if (!isbnField.getText().trim().matches("\\d{13}")) {
            errorMessage.append("ISBN must be a 13-digit number.\n");
        }
        ...
        // Validate the year field (should be a 4-digit number and realistic)
        String yearText = yearField.getText().trim();
        if (yearText.isEmpty()) {
            errorMessage.append("Year cannot be empty.\n");
        } else {
            try {
                int year = Integer.parseInt(yearText);
                int currentYear = java.time.Year.now().getValue();
                if (year < 1000 || year > currentYear) {
                    errorMessage.append("Year must be a valid 4-digit number, between 1000 and the current year.\n");
                }
        ...
        // Validate the price field (should be a positive number)
        ...
        // Return error message if any, otherwise return an empty string
